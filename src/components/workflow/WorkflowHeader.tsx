...
export function WorkflowHeader({ workflowId, onRun, onSave }: WorkflowHeaderProps) {
    ...
    return (
        <header className="absolute top-0 left-0 right-0 h-auto p-4 flex items-start justify-between z-20 pointer-events-none">
            {/* Left: Workflow Name (Wrapped in a pill for visibility since header is transparent) */}
            <div className="flex items-center pointer-events-auto bg-[#0E0E10] border border-[#1C1C1E] rounded-lg p-1">
                <input
                    type="text"
                    value={workflowName}
